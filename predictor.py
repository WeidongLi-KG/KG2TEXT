...
class Predictor(object):
    ...
    def preeval_batch(self, dataset):
        ...
        cands = {}
        i = 0
        total_target, total_out = [], []
        for batch_idx in range(len(dataset.corpus)):
            batch_s, batch_o_s, batch_f, batch_pf, batch_pb, sources, targets, fields, list_oovs, source_len, max_source_oov, w2fs = dataset.get_batch(batch_idx)
            ...
            for j in range(len(lengths)):
                ...
                total_target.append(targets[j])
                field[i] = ','.join(fields[j])
                values[i] = ','.join(sources[j])
                refs[i] = [ref]
                out_seq = []
                for k in range(lengths[j]):
                    symbol = decoded_outputs[j][k].item()
                    if symbol < self.vocab.size:
                        ...
                        out_seq.append(list_oovs[j][symbol-self.vocab.size])
                total_out.append(out_seq)
                out = self.prepare_for_bleu(out_seq)
                cands[i] = out

                # if i % 2500 == 0:
                #     print("Percentages:  %.4f" % (i/float(dataset.len)))
        return cands, refs, field, values, total_out, total_target
