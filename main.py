...
v_dataset = Table2text_seq(2, type=args.type, USE_CUDA=args.cuda, batch_size=config.batch_size)
...
def train_batch(dataset, batch_idx, model, teacher_forcing_ratio):
    batch_s, batch_o_s, batch_t, batch_o_t, batch_f, batch_pf, batch_pb, source_len, max_source_oov, batch_c = \
        dataset.get_batch(batch_idx)
    losses = model(batch_s, batch_o_s, max_source_oov, batch_f, batch_pf, batch_pb, source_len, batch_t,
                   batch_o_t, batch_c, teacher_forcing_ratio)
...
def calc_pcf(cands, refs, sources):
    total_num = 0
    appear_num = 0
    temp_num = 0
    for i, source in enumerate(sources):
        values = sources[i+1].split(',')
        for value in values:
            if value in cands[i]:
                temp_num += 1
            if value in refs[i]:
                total_num += 1
            if value in cands[i] and value in refs[i]:
                appear_num += 1
    precision = appear_num/temp_num
    recall = appear_num/total_num
...
def train_epoches(t_dataset, v_dataset, model, n_epochs, teacher_forcing_ratio):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        cand, ref, fields, sources, total_out, total_target = predictor.preeval_batch(v_dataset)
        print('Result:')
        print('field: ', fields[115])
        print('value: ', sources[115])
        print('ref: ', ref[115][0])
        print('cand: ', cand[115])
        final_scores = eval_f.evaluate(live=True, cand=cand, ref=ref)
        pcf = calc_pcf(total_out, total_target, sources)
        pcf = [epoch] + pcf
        result = pcf + [final_scores['Bleu_4'], final_scores['ROUGE_L'], final_scores['CIDEr']] \
                    + [epoch_loss] + [ref[115][0], cand[115]]
        results.append(result)
        pd.DataFrame(results, columns=['epoch', 'precision', 'recall', 'F1_score', 'Bleu_4', 'ROUGE_L', 'CIDEr', 'loss', 'reference', 'candidate']).to_csv(file_name)
