...
class Table2text_seq:
    ...
    def load_data(self, path):
        ...
        for idx, old_source in enumerate(old_sources):
            ...
            samples.append([source, target, field, p_for, p_bck, table, choose])
    ...
    def save2txt(self, path, data_set):
        file_name = os.path.splitext(path)[0] + '_sorted.txt'
        with open(file_name, 'w', encoding='utf-8') as file:
            for data in data_set:
                ...
                table = data[5]
                choose = data[6]
                file.write('field values: ' + '\n' + ', '.join(source) + '\n')
                file.write('field types: ' + '\n' + ', '.join(field) + '\n')
                file.write('position: ' + '\n' + ', '.join(str(i) for i in p_for) + '\n' + ', '.join(str(i) for i in p_bck) + '\n')
                file.write('target text: ' + '\n' + ', '.join(target) + '\n')
                file.write('value to field dict: ' + '\n' + json.dumps(table) + '\n')
                file.write('choose: \n' +  ', '.join(str(i) for i in choose) + '\n')
    ...
    def vectorize(self, sample):
        ...
        batch_c = [torch.FloatTensor(self.pad_vector(i, max(target_len))) for i in batch_c]

        batch_o_s = torch.stack(batch_o_s, dim=0) # (b, len)
        ...
        if self.mode != 0:
            ...
            return batch_s, batch_o_s, batch_t, batch_o_t, batch_f, batch_pf, batch_pb, source_len, max_source_oov, batch_c
