...
class DecoderRNN(BaseRNN):
    ...
    def decode_step(self, input_ids, coverage, _h, enc_proj, batch_size, max_enc_len,
        ...
        return combined_vocab, attn_scores, p_gen

    def forward(self, max_source_oov=0, targets=None, targets_id=None, choose=None, input_ids=None,
        ...
        if teacher_forcing_ratio:
            ...
            target_mask = targets_id.ne(0)[:, 1:].detach()
            ...
            for _step in range(max_length):
                ...
                combined_vocab, attn_scores, p_gen = self.decode_step(input_ids, coverage, _h, enc_proj, batch_size,
                ...
                ch_loss.append(p_gen)
                coverage = coverage + attn_scores

                # Coverage Loss
                coverage_loss.append(attn_scores * (1 - p_gen))
            ...
            coverage_loss = torch.stack(coverage_loss, dim=1) * target_mask.unsqueeze(-1).float()
            coverage_loss = coverage_loss.sum(1)
            ch_loss = torch.cat(ch_loss, 1) * target_mask.float()
            c_loss = torch.pow(coverage_loss - torch.ones_like(coverage).masked_fill_(enc_mask.byte(), 0), 2).mean(1)
            super_atten = self.bce_loss(ch_loss, choose[:, 1:])
            total_masked_loss = torch.cat(lm_loss, 1).sum(1).div(dec_lens) + c_loss
            total_loss = total_masked_loss.mean() + super_atten
            return total_loss
        else:
    ...
    def evaluate(self, targets, batch_size, max_length, max_source_oov, c_contexti, c_contextf, f_matrix,
        ...
        for _step in range(max_length):
            ...
            combined_vocab, attn_scores, p_gen = self.decode_step(input_ids, coverage,
